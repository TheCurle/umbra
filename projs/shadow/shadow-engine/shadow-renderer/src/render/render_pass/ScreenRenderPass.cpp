...
namespace vlkx {
    ...
    void ScreenRenderPassManager::initializeRenderPass() {
        ...
        passBuilder->updateAttachmentBacking(swapchainInfo.index.value(), [this](int index) -> const Image& {
            return *VulkanModule::getInstance()->getSwapchain()->images[index];
        });
    ...
    void ScreenRenderPassManager::preparePassBuilder() {
        ...
        swapchainInfo.add(tracker, *VulkanModule::getInstance()->getSwapchain()->images[0]);
        if (usesDepth)
        ...
        passBuilder = SimpleRenderPass::createBuilder(VulkanModule::getInstance()->getSwapchain()->images.size(), config, colorConfig, usesMultisampling ? &multisampleConfig : nullptr, usesDepth ? &depthConfig : nullptr, tracker);
    }
}
