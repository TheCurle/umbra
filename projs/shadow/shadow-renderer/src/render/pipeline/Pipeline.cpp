...
namespace vlkx {
    ...
    ShaderModule::ShaderModule(const std::string &path) {
        ...
        if (vkCreateShaderModule(VulkanManager::getInstance()->getDevice()->logical, &module, nullptr, &shader) != VK_SUCCESS)
    ...
    PipelineBuilder::PipelineBuilder(std::optional<int> maxCache) {
        ...
        if (vkCreatePipelineCache(VulkanManager::getInstance()->getDevice()->logical, &info, nullptr, &cache) != VK_SUCCESS)
    ...
    std::unique_ptr<Pipeline> GraphicsPipelineBuilder::build() const {
        ...
        if (vkCreatePipelineLayout(VulkanManager::getInstance()->getDevice()->logical, &getLayout(), nullptr, &pipelineLayout) != VK_SUCCESS)
        ...
        VkPipeline pipeline;
        if (vkCreateGraphicsPipelines(VulkanManager::getInstance()->getDevice()->logical, VK_NULL_HANDLE, 1, &createInfo, nullptr, &pipeline) != VK_SUCCESS)
    ...
    std::unique_ptr<Pipeline> ComputePipelineBuilder::build() const {
        ...
        if (vkCreatePipelineLayout(VulkanManager::getInstance()->getDevice()->logical, &getLayout(), nullptr,
                                   &layout) != VK_SUCCESS)
        ...
        VkPipeline pipeline;
        if (vkCreateComputePipelines(VulkanManager::getInstance()->getDevice()->logical, VK_NULL_HANDLE, 1, &createInfo,
