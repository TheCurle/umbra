...
VkDescriptorPool createPool(std::vector<Descriptor::Meta> metas) {
    ...
    if (vkCreateDescriptorPool(VulkanManager::getInstance()->getDevice()->logical, &create, nullptr, &pool) != VK_SUCCESS)
...
VkDescriptorSetLayout createLayout(std::vector<Descriptor::Meta> metas, bool dynamic) {
    ...
    if (vkCreateDescriptorSetLayout(VulkanManager::getInstance()->getDevice()->logical, &create, nullptr, &layout) != VK_SUCCESS)
...
VkDescriptorSet allocateSet(const VkDescriptorPool& pool, const VkDescriptorSetLayout& layout) {
    ...
    VkDescriptorSet set;
    if (vkAllocateDescriptorSets(VulkanManager::getInstance()->getDevice()->logical, &allocate, &set) != VK_SUCCESS)
...
const StaticDescriptor& StaticDescriptor::updateSet(const std::vector<VkWriteDescriptorSet> &write) const {
    vkUpdateDescriptorSets(VulkanManager::getInstance()->getDevice()->logical, write.size(), write.data(), 0, nullptr);
    return *this;
}
