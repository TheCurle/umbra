...
namespace vlkx {
    ...
    VkTools::ManagedImage createImage(const ImageConfig& config, VkImageCreateFlags flags, VkFormat format, const VkExtent3D& extent, VkImageUsageFlags usage) {
        const auto device = VulkanManager::getInstance()->getDevice();
        ...
        vmaCreateImage(VulkanManager::getInstance()->getAllocator(), &info, &allocateInfo, &image.image, &image.allocation, nullptr);
    ...
    DepthStencilImage::DepthStencilImage(const VkExtent2D &extent) : Image(extent, findFormatForDepthStencil()), buffer(extent, format) {
        setView(VkTools::createImageView(getImage(), format, VK_IMAGE_ASPECT_DEPTH_BIT | VK_IMAGE_ASPECT_STENCIL_BIT, 1, 1, VulkanManager::getInstance()->getDevice()->logical));
    }
    ...
    MultisampleImage::MultisampleImage(const VkExtent2D &extent, VkFormat format, vlkx::MultisampleImage::Mode mode,MultisampleBuffer::Type type)
        ...
        setView(VkTools::createImageView(buffer.getImage(), format, aspect, 1, 1, VulkanManager::getInstance()->getDevice()->logical));
    }

    VkSampleCountFlagBits MultisampleImage::chooseSamples(vlkx::MultisampleImage::Mode mode) {
        VkPhysicalDeviceLimits limits;
        VkPhysicalDeviceProperties props;
        vkGetPhysicalDeviceProperties(VulkanManager::getInstance()->getDevice()->physical, &props);
        limits = props.limits;
