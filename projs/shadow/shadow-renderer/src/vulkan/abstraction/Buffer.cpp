...
namespace vlkx {
    ...
    void StagingBuffer::copy(const VkBuffer &target) const {
        VkTools::copyGPUBuffer(get().buffer, target, dataSize, VulkanManager::getInstance()->getDevice());
    }
    ...
    UniformBuffer::UniformBuffer(size_t chunkSize, int chunks) : DataBuffer(), chunkSize(chunkSize), numChunks(chunks) {
        const VkDeviceSize alignment = VulkanManager::getInstance()->getDevice()->limits.minUniformBufferOffsetAlignment;
