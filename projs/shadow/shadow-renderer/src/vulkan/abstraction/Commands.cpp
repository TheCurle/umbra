...
VkCommandPool createPool(vlkx::Queue queue, bool shortLived) {
    ...
    // Create the pool
    if (vkCreateCommandPool(VulkanManager::getInstance()->getDevice()->logical, &poolCreateInfo, nullptr, &pool) != VK_SUCCESS)
...
std::vector<VkCommandBuffer> allocateBuffers(const VkCommandPool& pool, uint32_t amount) {
    ...
    if (vkAllocateCommandBuffers(VulkanManager::getInstance()->getDevice()->logical, &allocate, buffers.data()) != VK_SUCCESS)
...
vlkx::RenderCommand::RenderCommand(int frames) {
    VulkanDevice* dev = VulkanManager::getInstance()->getDevice();
    ...
    inFlight.resize(VulkanManager::getInstance()->getMaxFrames());
    VkFenceCreateInfo fenceInfo = {};
    fenceInfo.sType = VK_STRUCTURE_TYPE_FENCE_CREATE_INFO;
    fenceInfo.flags = VK_FENCE_CREATE_SIGNALED_BIT;

    for (size_t i = 0; i < VulkanManager::getInstance()->getMaxFrames(); i++) {
    ...
    setPool(pool);
    commands = allocateBuffers(pool, static_cast<uint32_t>(VulkanManager::getInstance()->getMaxFrames()));
}
...
std::optional<VkResult> vlkx::RenderCommand::execute(int frame, const VkSwapchainKHR &swapchain,
    ...
    const VkDevice& logical = VulkanManager::getInstance()->getDevice()->logical;
    ...
    return parseResult(vkQueuePresentKHR(VulkanManager::getInstance()->getDevice()->presentationQueue, &present));
}
